        ...
        if (jsonData.containsKey('products')) {
          ...
          if (products.isNotEmpty) {
            ...
            for (int i = 0; i < products.length && i < 5; i++) {
              ...
              print('${i + 1}. ${product['product_name'] ?? product['product_name_fr'] ?? 'No name'}');
              print('   Brands: ${product['brands'] ?? 'No brand'}');
              print('   Categories: ${product['categories'] ?? 'No categories'}');
              
              // Test the JSON structure that our parser expects
              final hasRequiredFields = product.containsKey('code') && 
                                      (product.containsKey('product_name') || product.containsKey('product_name_fr'));
            ...
            print('Wrapped product[\'product\'] keys: ${wrappedProduct['product'].keys.toList()}');
          }
        } else {
