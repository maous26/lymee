...
import 'package:lym_nutrition/core/error/exceptions.dart'
    hide Failure, ServerFailure, CacheFailure;
...
class FoodRepositoryImpl implements FoodRepository {
  ...
  Future<Either<Failure, List<FoodItem>>> searchFoods(
    ...
    try {
      ...
      final processedFoodsResult = await searchProcessedFoods(
        query,
        brand: brand,
      );
  ...
  @override
  Future<Either<Failure, List<FoodItem>>> searchProcessedFoods(
    String query, {
    String? brand,
  }) async {
    ...
    try {
      ...
      final localResults = await openFoodFactsLocalDataSource.searchFoods(
        query,
        brand: brand,
      );
      ...
      try {
        final remoteResults = await openFoodFactsRemoteDataSource.searchFoods(
          query,
          brand: brand,
        );
  ...
  @override
  Future<Either<Failure, FoodItem>> getFoodById(
    String id, {
    ...
    try {
      if (source == 'ciqual') {
        ...
        try {
          ...
          if (await networkInfo.isConnected) {
            final food = await openFoodFactsRemoteDataSource.getFoodByBarcode(
              id,
            );
            // Mettre en cache
    ...
    try {
      ...
      for (var item in historyItems) {
        if (item['source'] == 'ciqual') {
          ...
          try {
            final food = await openFoodFactsLocalDataSource.getFoodByBarcode(
              item['id'],
            );
            foods.add(food);
          } catch (_) {
