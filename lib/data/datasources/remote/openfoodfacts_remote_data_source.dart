...
abstract class OpenFoodFactsRemoteDataSource {
  ...
  Future<List<OpenFoodFactsFoodModel>> searchFoods(String query, {String? brand});
...
class OpenFoodFactsRemoteDataSourceImpl implements OpenFoodFactsRemoteDataSource {
  ...
  @override
  Future<List<OpenFoodFactsFoodModel>> searchFoods(String query, {String? brand}) async {
    // Construction de l'URL de recherche
    String url = 'https://world.openfoodfacts.org/cgi/search.pl?search_terms=${Uri.encodeComponent(query)}&json=1&page_size=50';
      ...
      if (response.statusCode == 200) {
        ...
        if (data.containsKey('products')) {
          ...
          for (int i = 0; i < products.length; i++) {
            try {
              final model = OpenFoodFactsFoodModel.fromJson({'product': products[i]});
        ...
        throw ServerException('Erreur lors de la recherche: ${response.statusCode}');
      }
    } catch (e) {
      print('  💥 Exception: $e');
  ...
  Future<OpenFoodFactsFoodModel> getFoodByBarcode(String barcode) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        throw ServerException('Erreur lors de la récupération: ${response.statusCode}');
      }
    } catch (e) {
      throw ServerException(e.toString());
    }
