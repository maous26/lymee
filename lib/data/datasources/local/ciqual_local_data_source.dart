...
class CiqualLocalDataSourceImpl implements CiqualLocalDataSource {
  ...
  Future<List<CiqualFoodModel>> searchFoods(String query) async {
    try {
      ...
      List<CiqualFoodModel> allFoods = decodedJson
          .map((item) => CiqualFoodModel.fromJson(item))
          .toList();
  ...
  Future<void> initializeDatabase() async {
    try {
      ...
      final String jsonString = await rootBundle.loadString('assets/data/common_ciqual.json');
      ...
      throw CacheException('Erreur lors de l\'initialisation de la base CIQUAL: ${e.toString()}');
    }
  }
  ...
  Future<void> clearCache() async {
    try {
      ...
      throw CacheException('Erreur lors de l\'effacement du cache CIQUAL: ${e.toString()}');
    }
  }
