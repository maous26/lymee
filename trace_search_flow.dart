    ...
    if (response.statusCode == 200) {
      ...
      if (jsonData.containsKey('products')) {
        ...
        if (products.isNotEmpty) {
          ...
          print('    Product name fr: ${firstProduct['product_name_fr'] ?? 'N/A'}');
...
void testURLConstruction() {
  ...
  for (final query in queries) {
    ...
    final url = 'https://world.openfoodfacts.org/cgi/search.pl?search_terms=$encodedQuery&json=1&page_size=50';
...
Future<void> testMultiLanguageSearch() async {
  ...
  for (final term in searchTerms) {
    ...
    try {
      final url = 'https://world.openfoodfacts.org/cgi/search.pl?search_terms=${Uri.encodeComponent(term)}&json=1&page_size=10';
      final client = http.Client();
      final response = await client.get(
        Uri.parse(url),
        headers: {
      ...
      if (response.statusCode == 200) {
        ...
        if (jsonData.containsKey('products')) {
          ...
          for (int i = 0; i < products.length && i < 3; i++) {
            ...
            final name = product['product_name'] ?? product['product_name_fr'] ?? 'Unknown';
            print('      ${i+1}. $name');
          }
        }
      }
